...
public class InternalFrameGestorDeposito extends JInternalFrame {
	...
	private void initialize() {
		...
		this.setTitle("Gestión de Depósitos");
		resetCampos();
	...
	public int deleteItem(Deposito deposito){
		int statusAction = 2; // 0=Exito | 1=Falso | 2=Error
		try{
			JOptionPane.showMessageDialog(null, "El Depósito Seleccionado Será Eliminado\n" +
					"Código: "+deposito.getCodigo()+"\n" +
					"Descripción: "+deposito.getDescripcion(),"Eliminar",JOptionPane.INFORMATION_MESSAGE);
			int selectedOption = JOptionPane.showConfirmDialog(null, "Desea Eliminar el Depósito","Eliminar", JOptionPane.OK_CANCEL_OPTION);
			if(selectedOption == 0){

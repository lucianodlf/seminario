...
public class InternalFrameGestorCondVenta extends JInternalFrame {

	private JPanel jContentPane = null;
	private static final long serialVersionUID = 1L;
	/*clases visuales*/
	private JScrollPane jScrollPaneaList = null;
	private MyJtable jTableCondicionVenta = null;
	...
	/*clases de logica*/
	private CondicionVenta condicionVenta = null;
	private CondicionVentaLogica condicionVentaLogica = CondicionVentaLogica.getInstance();  //  @jve:decl-index=0:
	private String activeActionType = null; //se utiliza para determina cual es la accion activa (agrega, modificar o eliminar)
	private JTextField jTextFieldCtasDefault = null;
	private JLabel jLabelCtasDefault = null;
	private JTextField jTextFieldDiasVto = null;
	private JLabel jLabelDiasVto = null;


	/**
	 * This is the xxx default constructor
	 */
	public InternalFrameGestorCondVenta() {
		super();
		initialize();
	}
	...
	private void initialize() {
		...
		this.setTitle("Gestión de Condición de Ventas");
		resetCampos();
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			jLabelDiasVto = new JLabel();
			jLabelDiasVto.setBounds(new Rectangle(185, 290, 130, 20));
			jLabelDiasVto.setText("Días de Vencimiento:");
			jLabelCtasDefault = new JLabel();
			jLabelCtasDefault.setBounds(new Rectangle(5, 290, 100, 20));
			jLabelCtasDefault.setText("Cant. de Cuotas:");
			...
			jContentPane.add(getJTextFieldCtasDefault(), null);
			jContentPane.add(jLabelCtasDefault, null);
			jContentPane.add(getJTextFieldDiasVto(), null);
			jContentPane.add(jLabelDiasVto, null);
		}
		return jContentPane;
	}
	...
	private JScrollPane getJScrollPaneaList() {
		if (jScrollPaneaList == null) {
			...
			jScrollPaneaList.setViewportView(getJtableCondicionVenta());
		}
		return jScrollPaneaList;
	}

	public MyJtable getJtableCondicionVenta() {
		/*
		 * Instanciamos el TableRowSorter y lo añadimos al JTable
		 */
		if (jTableCondicionVenta == null) {
			jTableCondicionVenta = new MyJtable(getMyTableModel());
			jTableCondicionVenta.setRowSorter(geTableRowSorter());
			jTableCondicionVenta.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
			jTableCondicionVenta.setRowHeight(20);
			setItemsInJtable();
			jTableCondicionVenta.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
			jTableCondicionVenta.setAutoscrolls(true);
			jTableCondicionVenta.setShowVerticalLines(true);
			jTableCondicionVenta.setShowHorizontalLines(true);
			jTableCondicionVenta.setVisible(true);
			jTableCondicionVenta.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(jTableCondicionVenta.isEnabled()){
						condicionVenta = getSelectedElement();
						if(condicionVenta != null){
							setCamposSelectedElement(condicionVenta);
			...
			jTableCondicionVenta.addKeyListener(new java.awt.event.KeyListener() {
	...
	public void setItemsInJtable(){
		ArrayList<CondicionVenta> condicionVentas = condicionVentaLogica.getListCondicionVenta();
		Vector<String> titulos = new Vector<String>();
		titulos.add("Código");
		titulos.add("Descripción");
		Vector<Vector<String>> registros = new Vector<Vector<String>>();
		Vector<String> renglon = null;
		if(condicionVentas != null){
			Iterator<CondicionVenta> it = condicionVentas.iterator();
			while(it.hasNext()){
				renglon = new Vector<String>();
				condicionVenta = (CondicionVenta)it.next();
				renglon.add(String.valueOf(condicionVenta.getCodigoCondicionVenta()));
				renglon.add(condicionVenta.getDescripcion());
				registros.add(renglon);
			}
		}
		myTableModel.setDataVector(registros, titulos);
		jTableCondicionVenta.getColumnModel().getColumn(0).setPreferredWidth(8);
		jTableCondicionVenta.getColumnModel().getColumn(1).setPreferredWidth(300);
	}

	/*
	 * Devuelve 1 item seleccionado de la lista.
	 */
	public CondicionVenta getSelectedElement(){
		try{
			int codigo = Integer.parseInt((String)myTableModel.getValueAt(tableRowSorter.convertRowIndexToModel(jTableCondicionVenta.getSelectedRow()), 0));
			condicionVenta = condicionVentaLogica.getCondicionVenta(codigo);
	...
	public void setCamposSelectedElement(CondicionVenta condicionVenta){
		jTextFieldCodigo.setText(String.valueOf(condicionVenta.getCodigoCondicionVenta()));
		jTextFieldDescripcion.setText(condicionVenta.getDescripcion());
		jTextFieldCtasDefault.setText(String.valueOf(condicionVenta.getCantidadCuotasDefault()));
		jTextFieldDiasVto.setText(String.valueOf(condicionVenta.getDiasVencimiento()));
	}
	...
	public int addItem(){
		...
		try{
			if(!jTextFieldDescripcion.getText().isEmpty()){
					...
					condicionVenta.setDescripcion(jTextFieldDescripcion.getText());
					if(!jTextFieldCtasDefault.getText().isEmpty())condicionVenta.setCantidadCuotasDefault(Integer.parseInt(jTextFieldCtasDefault.getText()));
					if(!jTextFieldDiasVto.getText().isEmpty())condicionVenta.setDiasVencimiento(Integer.parseInt(jTextFieldDiasVto.getText()));

					/*EJECUTA EL ADD CON LA LOGICA Y EVALUA LA RESPUESTA*/
					if(condicionVentaLogica.addCondicionVenta(condicionVenta)){
	...
	public int updateItem(CondicionVenta condicionVenta){
		int statusAction = 2; // 0=Exito | 1=Falso | 2=Error
		try{
			if(!jTextFieldDescripcion.getText().isEmpty()){
				/*setea los nuevos valores al objeto seleccionado para modificar*/

				condicionVenta.setCodigoCondicionVenta(Integer.parseInt(jTextFieldCodigo.getText()));
				condicionVenta.setDescripcion(jTextFieldDescripcion.getText());
				if(!jTextFieldCtasDefault.getText().isEmpty())condicionVenta.setCantidadCuotasDefault(Integer.parseInt(jTextFieldCtasDefault.getText()));
				if(!jTextFieldDiasVto.getText().isEmpty())condicionVenta.setDiasVencimiento(Integer.parseInt(jTextFieldDiasVto.getText()));

				/*EJECUTA EL UDPATE CON LA LOGICA*/
				if(condicionVentaLogica.updateCondicionVenta(condicionVenta)){
	...
	public int deleteItem(CondicionVenta condicionVenta){
		int statusAction = 2; // 0=Exito | 1=Falso | 2=Error
		try{
			JOptionPane.showMessageDialog(null, "La Condición de Venta seleccionada será Eliminada\n" +
					"Código: "+condicionVenta.getCodigoCondicionVenta()+"\n" +
					"Descripción: "+condicionVenta.getDescripcion(),"Eliminar",JOptionPane.INFORMATION_MESSAGE);
			int selectedOption = JOptionPane.showConfirmDialog(null, "Desea Eliminar la Condición de Venta Seleccionada","Eliminar", JOptionPane.OK_CANCEL_OPTION);
			if(selectedOption == 0){
				/*EJECUTA EL DELETE CON LA LOGICA*/
				if(condicionVentaLogica.deleteCondicionVenta(condicionVenta.getCodigoCondicionVenta())){
					/*SI ES SATISFACTORIO EL UPDATE*/
					 JOptionPane.showMessageDialog(null, "Operación Satisfactoria!", "Operación Satisfactoria", JOptionPane.INFORMATION_MESSAGE);
					 statusAction = 0;
				}else{
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if(activeActionType.equals("ADD")){
						...
						if(statusAction == 0){
							...
							if(selectedOption == 0){
								jTextFieldCodigo.setText(String.valueOf(condicionVentaLogica.getNewCodigoItems()));
								jTextFieldDescripcion.requestFocus(true);
							}else{
	...
	public void setEstatusAllComponent(boolean estado, boolean estadoTextBusqueda, boolean estadoComboBoxBusqueda, boolean estadoTableList, boolean estadoBtnSeleccionar){
		...
		jTableCondicionVenta.setEnabled(estadoTableList);
	}
	...
	/**
	 * This method initializes jTextFieldCtasDefault
	 *
	 * @return javax.swing.JTextField
	 */
	private JTextField getJTextFieldCtasDefault() {
		if (jTextFieldCtasDefault == null) {
			jTextFieldCtasDefault = new JTextField();
			jTextFieldCtasDefault.setBounds(new Rectangle(110, 290, 50, 20));
			jTextFieldCtasDefault.setDocument(new MyPlainDocument(getJTextFieldCtasDefault(), 50, null, true));
		}
		return jTextFieldCtasDefault;
	}

	/**
	 * This method initializes jTextFieldDiasVto
	 *
	 * @return javax.swing.JTextField
	 */
	private JTextField getJTextFieldDiasVto() {
		if (jTextFieldDiasVto == null) {
			jTextFieldDiasVto = new JTextField();
			jTextFieldDiasVto.setBounds(new Rectangle(320, 290, 50, 20));
			jTextFieldDiasVto.setDocument(new MyPlainDocument(getJTextFieldDiasVto(), 50, null, true));
		}
		return jTextFieldDiasVto;
	}

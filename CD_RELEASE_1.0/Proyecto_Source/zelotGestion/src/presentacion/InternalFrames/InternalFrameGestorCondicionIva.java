...
public class InternalFrameGestorCondicionIva extends JInternalFrame {

	private JPanel jContentPane = null;
	private static final long serialVersionUID = 1L;
	/*clases visuales*/
	private JScrollPane jScrollPaneaList = null;
	private MyJtable jTableCondicionIva = null;
	...
	private JCheckBox jCheckBoxMuestraIVA = null;
	private JCheckBox jCheckBoxDiscriminaIVA = null;
	 ...
	 */
	public InternalFrameGestorCondicionIva() {
		super();
		initialize();
	}
	...
	private void initialize() {
		...
		this.setTitle("Gestión de Condiciones de IVA");
		resetCampos();
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jLabelLetra.setBounds(new Rectangle(05, 280, 40, 20));
			...
			jContentPane.add(jLabelLetra, null);
			jContentPane.add(getJTextFieldCodigo(), null);
			jContentPane.add(getJTextFieldDescripcion(), null);
			jContentPane.add(getJTextFieldLetra(), null);
			jContentPane.add(getJCheckBoxMuestraIVA(), null);
			jContentPane.add(getJCheckBoxDiscriminaIVA(), null);
	...
	private JScrollPane getJScrollPaneaList() {
		if (jScrollPaneaList == null) {
			...
			jScrollPaneaList.setViewportView(getJtableCondicionIva());
		}
		return jScrollPaneaList;
	}

	public MyJtable getJtableCondicionIva() {
		/*
		 * Instanciamos el TableRowSorter y lo añadimos al JTable
		 */
		if (jTableCondicionIva == null) {
			jTableCondicionIva = new MyJtable(getMyTableModel());
			jTableCondicionIva.setRowSorter(geTableRowSorter());
			jTableCondicionIva.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
			jTableCondicionIva.setRowHeight(20);
			setItemsInJtable();
			jTableCondicionIva.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
			jTableCondicionIva.setAutoscrolls(true);
			jTableCondicionIva.setShowVerticalLines(true);
			jTableCondicionIva.setShowHorizontalLines(true);
			jTableCondicionIva.setVisible(true);
			jTableCondicionIva.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(jTableCondicionIva.isEnabled()){
						condicionIva = getSelectedElement();
						if(condicionIva != null){
							setCamposSelectedElement(condicionIva);
			...
			jTableCondicionIva.addKeyListener(new java.awt.event.KeyListener() {
	...
	private JTextField getJTextFieldLetra() {
		if (jTextFieldLetra == null) {
			jTextFieldLetra = new JTextField();
			jTextFieldLetra.setBounds(new Rectangle(65, 280, 50, 20));
			jTextFieldLetra.setDocument(new MyPlainDocument(jTextFieldLetra,1,"L",true));
		}
		return jTextFieldLetra;
	}

	private JCheckBox getJCheckBoxMuestraIVA() {
		if (jCheckBoxMuestraIVA == null) {
			jCheckBoxMuestraIVA = new JCheckBox();
			jCheckBoxMuestraIVA.setBounds(new Rectangle(465, 290, 103, 20));
			jCheckBoxMuestraIVA.setText("Muestra IVA");
		}
		return jCheckBoxMuestraIVA;
	}

	private JCheckBox getJCheckBoxDiscriminaIVA() {
		if (jCheckBoxDiscriminaIVA == null) {
			jCheckBoxDiscriminaIVA = new JCheckBox();
			jCheckBoxDiscriminaIVA.setBounds(new Rectangle(465, 255, 115, 20));
			jCheckBoxDiscriminaIVA.setText("Discrimina IVA");
		}
		return jCheckBoxDiscriminaIVA;
	}
	...
	public void setItemsInJtable(){
		ArrayList<CondicionIva> condicionIvas = condicionIvaLogica.getListCondicionIva();
		Vector<String> titulos = new Vector<String>();
		titulos.add("Código");
		titulos.add("Descripción");
		titulos.add("Letra Fac.");
		Vector<Vector<String>> registros = new Vector<Vector<String>>();
		Vector<String> renglon = null;
		if(condicionIvas != null){
			Iterator<CondicionIva> it = condicionIvas.iterator();
			while(it.hasNext()){
				renglon = new Vector<String>();
				condicionIva = (CondicionIva)it.next();
				renglon.add(String.valueOf(condicionIva.getCodigo()));
				renglon.add(condicionIva.getDescripcion());
				renglon.add(condicionIva.getLetraFacturacionDefault());
				registros.add(renglon);
			}
		}
		myTableModel.setDataVector(registros, titulos);
		//setea ancho de campos
		jTableCondicionIva.getColumnModel().getColumn(0).setPreferredWidth(8);
		jTableCondicionIva.getColumnModel().getColumn(1).setPreferredWidth(8);
	}

	/*
	 * Devuelve 1 item seleccionado de la lista.
	 */
	public CondicionIva getSelectedElement(){
		try{
			int codigo = Integer.parseInt((String)myTableModel.getValueAt(tableRowSorter.convertRowIndexToModel(jTableCondicionIva.getSelectedRow()), 0));
			condicionIva = condicionIvaLogica.getCondicionIva(codigo);
			return condicionIva;
	...
	public void setCamposSelectedElement(CondicionIva condicionIva){
		jTextFieldCodigo.setText(String.valueOf(condicionIva.getCodigo()));
		jTextFieldDescripcion.setText(condicionIva.getDescripcion());
		jTextFieldLetra.setText(condicionIva.getLetraFacturacionDefault());
		jCheckBoxDiscriminaIVA.setSelected(condicionIva.isDiscriminaIVA());
		jCheckBoxMuestraIVA.setSelected(condicionIva.isMuestraIVA());
	}
	...
	public int addItem(){
		...
		try{
			if(jTextFieldDescripcion.getText().length() != 0 && jTextFieldLetra.getText().length() != 0){

					/*CREA EL OBJETO Y SETEA LOS VALORES*/
					condicionIva = new CondicionIva();
					condicionIva.setDescripcion(jTextFieldDescripcion.getText());
					condicionIva.setDiscriminaIVA(jCheckBoxDiscriminaIVA.isSelected());
					condicionIva.setMuestraIVA(jCheckBoxMuestraIVA.isSelected());
					condicionIva.setLetraFacturacionDefault(jTextFieldLetra.getText());

					/*EJECUTA EL ADD CON LA LOGICA Y EVALUA LA RESPUESTA*/
					if(condicionIvaLogica.addCondicionIva(condicionIva)){
	...
	public int updateItem(CondicionIva condicionIva){
		int statusAction = 2; // 0=Exito | 1=Falso | 2=Error
		try{
			if(jTextFieldDescripcion.getText().length() != 0 && jTextFieldLetra.getText().length() != 0){
				/*setea los nuevos valores al objeto seleccionado para modificar*/
				condicionIva.setCodigo(Integer.parseInt(jTextFieldCodigo.getText()));
				condicionIva.setDescripcion(jTextFieldDescripcion.getText());
				condicionIva.setDiscriminaIVA(jCheckBoxDiscriminaIVA.isSelected());
				condicionIva.setMuestraIVA(jCheckBoxMuestraIVA.isSelected());
				condicionIva.setLetraFacturacionDefault(jTextFieldLetra.getText());
				...
				if(condicionIvaLogica.updateCondicionIva(condicionIva)){
	...
	public int deleteItem(CondicionIva condicionIva){
		int statusAction = 2; // 0=Exito | 1=Falso | 2=Error
		try{
			JOptionPane.showMessageDialog(null, "La Condición de IVA Seleccionada Será Eliminada\n" +
					"Código: "+condicionIva.getCodigo()+"\n" +
					"Descripción: "+condicionIva.getDescripcion(),"Eliminar",JOptionPane.INFORMATION_MESSAGE);
			int selectedOption = JOptionPane.showConfirmDialog(null, "Desea Eliminar la Condición de IVA","Eliminar", JOptionPane.OK_CANCEL_OPTION);
			if(selectedOption == 0){
				/*EJECUTA EL DELETE CON LA LOGICA*/
				if(condicionIvaLogica.deleteCondicionIva(condicionIva.getCodigo())){
					/*SI ES SATISFACTORIO EL UPDATE*/
					 JOptionPane.showMessageDialog(null, "Operación Satisfactoria!", "Operación Satisfactoria", JOptionPane.INFORMATION_MESSAGE);
					 statusAction = 0;
				}else{
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if(activeActionType.equals("ADD")){
						...
						if(statusAction == 0){
							...
							if(selectedOption == 0){
								jTextFieldCodigo.setText(String.valueOf(condicionIvaLogica.getNewCodigoItems()));
								jTextFieldDescripcion.requestFocus(true);
							}else{
	...
	public void setEstatusAllComponent(boolean estado, boolean estadoTextBusqueda, boolean estadoComboBoxBusqueda, boolean estadoTableList){
		...
		jTableCondicionIva.setEnabled(estadoTableList);
	}

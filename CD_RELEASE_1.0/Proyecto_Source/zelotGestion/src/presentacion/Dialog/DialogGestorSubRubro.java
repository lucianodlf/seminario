...
public class DialogGestorSubRubro extends JDialog {

	private JPanel jContentPane = null;
	private static final long serialVersionUID = 1L;
	/*clases visuales*/
	private JScrollPane jScrollPaneaList = null;
	private MyJtable jTableSubRubro = null;
	...
	private SubRubroLogica subRubroLogica = SubRubroLogica.getInstance();  //  @jve:decl-index=0:
	 ...
	 */
	public DialogGestorSubRubro(Frame owner) {
		super(owner);
		initialize();
	}
	...
	private void initialize() {
		...
		this.setTitle("Gestión de SubRubros");
		resetCampos();
	...
	private JScrollPane getJScrollPaneaList() {
		if (jScrollPaneaList == null) {
			...
			jScrollPaneaList.setViewportView(getJtableSubRubro());
		}
		return jScrollPaneaList;
	}

	public MyJtable getJtableSubRubro() {
		/*
		 * Instanciamos el TableRowSorter y lo añadimos al JTable
		 */
		if (jTableSubRubro == null) {
			jTableSubRubro = new MyJtable(getMyTableModel());
			jTableSubRubro.setRowSorter(geTableRowSorter());
			jTableSubRubro.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
			jTableSubRubro.setRowHeight(20);
			setItemsInJtable();
			jTableSubRubro.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
			jTableSubRubro.setAutoscrolls(true);
			jTableSubRubro.setShowVerticalLines(true);
			jTableSubRubro.setShowHorizontalLines(true);
			jTableSubRubro.setVisible(true);
			jTableSubRubro.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(jTableSubRubro.isEnabled()){
						subRubro = getSelectedElement();
						if(subRubro != null){
							setCamposSelectedElement(subRubro);
			...
			jTableSubRubro.addKeyListener(new java.awt.event.KeyListener() {
	...
	private JComboBox getJComboBoxCriterioBusqueda() {
		if (jComboBoxCriterioBusqueda == null) {
			...
			jComboBoxCriterioBusqueda.addItem("Codigo");
			jComboBoxCriterioBusqueda.addItem("Descripcion");
		}
	...
	public void setItemsInJtable(){
		ArrayList<SubRubro> subRubros = subRubroLogica.getListSubRubro();
		...
		if(subRubros != null){
			Iterator<SubRubro> it = subRubros.iterator();
			while(it.hasNext()){
				renglon = new Vector<String>();
				subRubro = (SubRubro)it.next();
				renglon.add(String.valueOf(subRubro.getCodigo()));
				renglon.add(subRubro.getDescripcion());
				registros.add(renglon);
			}
		}
		myTableModel.setDataVector(registros, titulos);
		getJtableSubRubro().getColumnModel().getColumn(0).setPreferredWidth(8);
	}

	/*
	 * Devuelve 1 item seleccionado de la lista.
	 */
	public SubRubro getSelectedElement(){
		try{
			int codigo = Integer.parseInt((String)myTableModel.getValueAt(tableRowSorter.convertRowIndexToModel(jTableSubRubro.getSelectedRow()), 0));
			subRubro = subRubroLogica.getSubRubro(codigo);
			return subRubro;
	...
	public void setCamposSelectedElement(SubRubro subRubro){
		jTextFieldCodigo.setText(String.valueOf(subRubro.getCodigo()));
		jTextFieldDescripcion.setText(subRubro.getDescripcion());
	}
	...
	public int addItem(){
		...
		try{
			if(jTextFieldDescripcion.getText().length() != 0 ){
					...
					subRubro.setDescripcion(jTextFieldDescripcion.getText());

					/*EJECUTA EL ADD CON LA LOGICA Y EVALUA LA RESPUESTA*/
					if(subRubroLogica.addSubRubro(subRubro)){
	...
	public int updateItem(SubRubro subRubro){
		int statusAction = 2; // 0=Exito | 1=Falso | 2=Error
		try{
			if(jTextFieldDescripcion.getText().length() != 0){
				/*setea los nuevos valores al objeto seleccionado para modificar*/
				subRubro.setCodigo(Integer.parseInt(jTextFieldCodigo.getText()));
				subRubro.setDescripcion(jTextFieldDescripcion.getText());

				/*EJECUTA EL UDPATE CON LA LOGICA*/
				if(subRubroLogica.updateSubRubro(subRubro)){
	...
	public int deleteItem(SubRubro subRubro){
		int statusAction = 2; // 0=Exito | 1=Falso | 2=Error
		try{
			JOptionPane.showMessageDialog(null, "El SubRubro Seleccionado Será Eliminado\n" +
					"Código: "+subRubro.getCodigo()+"\n" +
					"Descripción: "+subRubro.getDescripcion(),"Eliminar",JOptionPane.INFORMATION_MESSAGE);
			int selectedOption = JOptionPane.showConfirmDialog(null, "Desea Eliminar el SubRubro","Eliminar", JOptionPane.OK_CANCEL_OPTION);
			if(selectedOption == 0){
				/*EJECUTA EL DELETE CON LA LOGICA*/
				if(subRubroLogica.deleteSubRubro(subRubro.getCodigo())){
					/*SI ES SATISFACTORIO EL UPDATE*/
					 JOptionPane.showMessageDialog(null, "Operación Satisfactoria!", "Operación Satisfactoria", JOptionPane.INFORMATION_MESSAGE);
					 statusAction = 0;
				}else{
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if(activeActionType.equals("ADD")){
						...
						if(statusAction == 0){
							...
							if(selectedOption == 0){
								jTextFieldCodigo.setText(String.valueOf(subRubroLogica.getNewCodigoItems()));
								jTextFieldDescripcion.requestFocus(true);
							}else{
	...
	public void setEstatusAllComponent(boolean estado, boolean estadoTextBusqueda, boolean estadoComboBoxBusqueda, boolean estadoTableList, boolean estadoBtnSeleccionar){
		...
		jTableSubRubro.setEnabled(estadoTableList);

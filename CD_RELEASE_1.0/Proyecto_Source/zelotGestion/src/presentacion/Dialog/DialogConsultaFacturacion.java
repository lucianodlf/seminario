...
public class DialogConsultaFacturacion extends JDialog {
	 ...
	 */
	public DialogConsultaFacturacion(Frame owner) {
		super(owner);
		initialize();
	}

	public DialogConsultaFacturacion(Frame owner, String fechaD, String fechaH, Cliente cli) {
		super(owner);
	...
	private void initialize() {
		...
		if(!fechaDesde.isEmpty() && !fechaHasta.isEmpty() && cliente != null){
				...
				jTextFieldBusqueda.setText("FAC");
				tableRowSorter.setRowFilter(RowFilter.regexFilter("FAC", 1));
		}
		try {
	...
	private JComboBox getJComboBoxCriterioBusqueda() {
		if (jComboBoxCriterioBusqueda == null) {
			...
			jComboBoxCriterioBusqueda.addItem("Típo Comp.");
	...
	public void setFiltroByCliente(Cliente cli){
		if(cli != null){
			...
			jCheckBoxFiltraCliente.setSelected(true);
			this.selectComprobantes(fechaHasta, fechaDesde, cliente);
		}
	}
